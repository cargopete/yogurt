...
// Field offsets for EthereumCall
mod call_offsets {
    pub const TO: usize = 0;
    pub const FROM: usize = 4;
    pub const BLOCK: usize = 8;
    pub const TRANSACTION: usize = 12;
    pub const INPUT_VALUES: usize = 16;
    pub const OUTPUT_VALUES: usize = 20;
}
...
/// An Ethereum function call with typed inputs and outputs.
///
/// Used by call handlers to receive the function call context and data.
/// The `I` type contains the decoded function inputs, and `O` contains
/// the decoded function outputs.
#[derive(Clone, Debug)]
pub struct Call<I, O> {
    pub to: Address,
    pub from: Address,
...
/// Deserialize a Call from an AssemblyScript pointer.
///
/// The input type `I` and output type `O` must implement `FromAscPtr` —
/// these are typically generated by `yogurt codegen` based on the ABI.
#[cfg(target_arch = "wasm32")]
impl<I: FromAscPtr, O: FromAscPtr> FromAscPtr for Call<I, O> {
    fn from_asc_ptr(ptr: u32) -> Self {
        use crate::asc::read_u32_at;

        if ptr == 0 {
            panic!("Cannot deserialize Call from null pointer");
        }

        unsafe {
            Call {
                to: Address::from_asc_ptr(read_u32_at(ptr, call_offsets::TO)),
                from: Address::from_asc_ptr(read_u32_at(ptr, call_offsets::FROM)),
                block: Block::from_asc_ptr(read_u32_at(ptr, call_offsets::BLOCK)),
                transaction: Transaction::from_asc_ptr(read_u32_at(ptr, call_offsets::TRANSACTION)),
                inputs: I::from_asc_ptr(read_u32_at(ptr, call_offsets::INPUT_VALUES)),
                outputs: O::from_asc_ptr(read_u32_at(ptr, call_offsets::OUTPUT_VALUES)),
            }
        }
    }
}
...
impl<I: FromAscPtr, O: FromAscPtr> FromAscPtr for Call<I, O> {
    fn from_asc_ptr(_ptr: u32) -> Self {
        panic!("Call deserialization not available on native target")
    }
}
